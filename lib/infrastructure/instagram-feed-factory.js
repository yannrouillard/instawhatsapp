...
class InstagramFeed {
  ...
  _minimizePost(mediaPost) {
    const getMedia = post => (post.media_type === MEDIA_CAROUSEL ? post.carousel_media : [post]);
    ...
    const keepOneResolution = media => {
      return media.media_type === MEDIA_VIDEO
        ? media.video_versions2.candidates[0]
    ...
    return {
      ...
      title: mediaPost.caption.text
      timestamp: mediaPost.taken_at,
      mediaUrls: getMedia(mediaPost)
        .map(keepOneResolution)
        .map((media) => media.url),
    };
  }
}
