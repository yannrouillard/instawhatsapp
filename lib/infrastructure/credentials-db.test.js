...
test('getPassword raise an exception if file is not found', async () => {
  ...
  const credentialsDb = new CredentialsDb(CREDENTIALS_FOLDER);
  // When and Then
  const throwingPromise = credentialsDb.getPassword(FAKE_ACCOUNT_NAME);
  await expect(throwingPromise).rejects.toThrow(MissingPasswordError);
});
...
test('getPassword raise an exception if invalid format', async () => {
  ...
  // When and Then
  const throwingPromise = credentialsDb.getPassword(FAKE_ACCOUNT_NAME);
  await expect(throwingPromise).rejects.toThrow(InvalidFormatError);
});
