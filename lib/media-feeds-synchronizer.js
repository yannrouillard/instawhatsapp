...
class MediaFeedsSynchronizer extends EventEmitter {
  ...
  async synchronizeMediaFeeds({ sourceFeedInfo, targetFeedInfo, options = {} }) {
    ...
    const { skip = 0, dryRun, max, since = lastTimestamp } = options;

    this.emit('synchronizationStarting', sourceFeedInfo, targetFeedInfo);
    const posts = await this.getMediaPostsFromSource(sourceFeedInfo, { skip, since, max });
    await this.sendMediaPostsToTarget(posts, targetFeedInfo, { dryRun });
    this.emit('synchronizationFinished', sourceFeedInfo, targetFeedInfo);
  }
