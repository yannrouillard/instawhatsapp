...
const InstagramFeedFactoryMock = require('../lib/infrastructure/instagram-feed-factory');
const WhatsAppFeedFactoryMock = require('../lib/infrastructure/whatsapp-feed-factory');

const MediaFeedInfo = require('./media-feed-info');
const MediaFeedsSynchronizer = require('./media-feeds-synchronizer');
const SynchroStateDbMock = require('../lib/infrastructure/synchronization-state-db');
...
test('synchronizeMediaFeeds starts from the last synchronisation timestamp', async () => {
  ...
  expect(sentMediaPosts.length).toEqual(FAKE_INSTAGRAM_POSTS_COUNT - lastSynchroTimestamp);
});
...
test('synchronizeMediaFeeds do not send any message in dry run mode', async () => {
  ...
  // Then
  const sentMediaPosts = WhatsAppFeedFactoryMock.getSentMediaPosts(FAKE_WHATSAPP_FEED_INFO);
  expect(sentMediaPosts.length).toEqual(0);
});
...
test('synchronizeMediaFeeds do not send messages skipped', async () => {
  ...
  expect(sentMediaPosts.length).toEqual(FAKE_INSTAGRAM_POSTS_COUNT - skippedPosts);
});
